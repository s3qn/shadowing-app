...
def test_posting_one_practice_event_creates_a_level_one_row_due_tomorrow():
    result = asyncio.run(schedule.log_practice(island_id="island-a", seconds=30.0, authorization=AUTH))
...
def test_posting_twice_moves_the_island_to_level_two():
    asyncio.run(schedule.log_practice(island_id="island-b", seconds=30.0, authorization=AUTH))
    result = asyncio.run(schedule.log_practice(island_id="island-b", seconds=30.0, authorization=AUTH))

    assert result["level"] == 2


def test_due_today_includes_an_island_due_now_and_omits_one_due_next_week():
    asyncio.run(schedule.log_practice(island_id="due-soon", seconds=10.0, authorization=AUTH))
    # Bump this island to a level whose interval is a week or more out.
    for _ in range(5):
        asyncio.run(schedule.log_practice(island_id="due-later", seconds=10.0, authorization=AUTH))

    due = asyncio.run(schedule.due_today(authorization=AUTH))
...
def test_zero_or_negative_seconds_is_a_400():
    with pytest.raises(Exception) as exc_info:
        asyncio.run(schedule.log_practice(island_id="island-c", seconds=0, authorization=AUTH))
    assert getattr(exc_info.value, "status_code", None) == 400


def test_missing_token_on_practice_is_401():
    with pytest.raises(Exception) as exc_info:
        asyncio.run(schedule.log_practice(island_id="island-d", seconds=10.0, authorization=None))
    assert getattr(exc_info.value, "status_code", None) == 401


def test_bad_token_on_due_today_is_401():
    with pytest.raises(Exception) as exc_info:
        asyncio.run(schedule.due_today(authorization="Bearer wrong"))
    assert getattr(exc_info.value, "status_code", None) == 401
