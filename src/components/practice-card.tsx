...
export function PracticeCard({ log }: { log: PracticeLog }) {
  ...
  return (
    <View style={StyleSheet.flatten([styles.card, { backgroundColor: palette.surface, borderColor: palette.line }])}>
      <View style={styles.numbers}>
        <View style={styles.stat}>
          <Text style={[styles.value, { color: palette.ink }]}>{todayMinutes}</Text>
          <Text style={[styles.label, { color: palette.muted }]}>Today</Text>
        </View>
        <View style={styles.stat}>
          <Text style={[styles.value, { color: palette.ink }]}>{streak}</Text>
          <Text style={[styles.label, { color: palette.muted }]}>Streak</Text>
        </View>
      </View>
        ...
        {week.map((key) => {
          ...
          return (
            <View key={key} style={styles.day}>
              <Text style={[styles.dayLetter, { color: palette.muted }]}>{letter}</Text>
              <Text style={[styles.dayValue, { color: palette.ink }]}>{minutes > 0 ? minutes : '·'}</Text>
            </View>
          );
        })}
      </View>
